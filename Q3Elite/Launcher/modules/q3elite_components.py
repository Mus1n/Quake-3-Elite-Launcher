...
def _extract_basic_zip(zip_path, group=None, control=None, progress_callback=None,
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for info, rel, dest in members:
            if control is not None and not control.wait_if_paused():
                raise RuntimeError("Basic ZIP extraction cancelled.")
...
def _extract_basic_maps_zip(zip_path, group, control=None):
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for info in zf.infolist():
            if info.is_dir():
                continue
            if control is not None and not control.wait_if_paused():
                raise RuntimeError("Basic Maps ZIP extraction cancelled.")
...
def _bulk_install_basic_maps(files, control=None, progress_callback=None):
    ...
    return _extract_basic_maps_zip(archive, group, control)
