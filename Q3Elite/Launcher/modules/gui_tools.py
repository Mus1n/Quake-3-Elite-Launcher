...
class Terminal(QTextEdit):
    ...
    def __init__(self, parent=None, redirect_stdout=True, redirect_stderr=False,
                 background_path=None, darken=0, text_color="#000000", font=None):
        super().__init__(parent)
        ...
        if font == None:
            ...
            self.setStyleSheet(f"QTextEdit {{ background: transparent; color: {text_color}; border: 0px; font-family: {font}; }}")
    ...
    def set_font(self, font):
        self.setStyleSheet(f"QTextEdit {{ background: transparent; color: {self.text_color}; border: 0px; font-family: {font}; }}")
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("Q3Elite")
        ...
        self.launch_2.currentChanged.connect(self.on_tab_changed)

        self.tableWidget = ModTableWidget(
            parent=self.mod_repo,
            background_path='./assets/tback.png',
            darken=140
        )
        ...
        self.terminal = Terminal(
            ...
            background_path='./assets/back.png',
            darken=140,
            text_color='#ffffff'
        )
        ...
        self.label_logo.setGeometry(QtCore.QRect(0, 0, 900, 600))  # подберите координаты
        pixmap = QPixmap("./assets/back.png")
        self.label_logo.setPixmap(pixmap)
        self.label_logo.setObjectName("label_logo")
        self.label_logo.setPixmap(load_cover_pixmap(
            './assets/back.png', 900, 600
        ))
        ...
        # панель вкладок должна рисоваться поверх содержимого страниц,
        # т.к. QTabWidget::pane сдвинут под неё отрицательным margin в style.css
        self.launch_2.tabBar().raise_()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.launch_2.setTabIcon(self.launch_2.indexOf(self.launch_1), load_tab_icon("./assets/b3.png"))
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Quake 3 Elite"))
        self.pushButton.setText(_translate("MainWindow", "Checking..."))
        self.pushButton.setEnabled(False)
        
    def on_tab_changed(self, index: int):
        if index == 0:
            self.tab_hides(False)
        elif index == 1:
            self.tab_hides(True)

    def _init_tabbar_hover(self):
        """Ленивая инициализация геометрий и таймера для скрытия/показа
        панели вкладок по наведению курсора. Вызывается один раз при
        первом hides(...)."""
        if getattr(self, "_tabbar_hover_ready", False):
            return
    
        tab_bar = self.launch_2.tabBar()
        shown_geo = tab_bar.geometry()
        # прячем панель за левый край окна, оставляя узкую полоску-подсказку
        hidden_geo = QtCore.QRect(
            -(shown_geo.width() - 6), shown_geo.y(),
            shown_geo.width(), shown_geo.height()
        )
    
        self._tabbar_shown_geo = shown_geo
        self._tabbar_hidden_geo = hidden_geo
        self._tabbar_visible = True
        self._tabbar_hover_zone = 40  # ширина зоны наведения слева, px
    
        self._tabbar_anim = QtCore.QPropertyAnimation(tab_bar, b"geometry")
        self._tabbar_anim.setDuration(150)
        self._tabbar_anim.setEasingCurve(QtCore.QEasingCurve.Type.OutCubic)
    
        self._tabbar_hover_timer = QtCore.QTimer(self.centralwidget)
        self._tabbar_hover_timer.setInterval(60)
        self._tabbar_hover_timer.timeout.connect(self._check_tabbar_hover)
    
        self._tabbar_hover_ready = True
        self._tabbar_hover_enabled = False
    
    def _check_tabbar_hover(self):
        tab_bar = self.launch_2.tabBar()
        pos = self.centralwidget.mapFromGlobal(QtGui.QCursor.pos())
        near_edge = (
            self.centralwidget.rect().contains(pos)
            and pos.x() <= self._tabbar_hover_zone
        )
        if near_edge == self._tabbar_visible:
            return
        self._tabbar_visible = near_edge
        target = self._tabbar_shown_geo if near_edge else self._tabbar_hidden_geo
        self._tabbar_anim.stop()
        self._tabbar_anim.setStartValue(tab_bar.geometry())
        self._tabbar_anim.setEndValue(target)
        self._tabbar_anim.start()
    
    def tab_hides(self, state):
        """hides(True)  — панель вкладок прячется за левый край окна и
                            выезжает только когда курсор наведён на край.
            hides(False) — панель вкладок всегда видна как обычно."""
        self._init_tabbar_hover()
        tab_bar = self.launch_2.tabBar()
    
        if state:
            self._tabbar_hover_enabled = True
            self._tabbar_visible = True  # заставит таймер спрятать её при первой проверке
            self._tabbar_hover_timer.start()
            self._check_tabbar_hover()
        else:
            self._tabbar_hover_enabled = False
            self._tabbar_hover_timer.stop()
            self._tabbar_anim.stop()
            tab_bar.setGeometry(self._tabbar_shown_geo)
            self._tabbar_visible = True
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def launch(self):
        ...
        if launch(force_ogl=is_opengl_forced):
            self.close()
        else:
            ToastNotification.show_warning(self.centralwidget, 'Launch error!\nOpenGL engine was not found.')
