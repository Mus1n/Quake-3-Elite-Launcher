...
class bmod:
    ...
    def __init__(self):
        with open(f"./mod_tree/{c_info.mod_branch}.bmod", 'r') as mod_branch:
            ...
            version_file = LAUNCHER_DIR / "version"
            sversion_file = LAUNCHER_DIR / "sversion"

            with open(version_file, 'r') as vfile, open(sversion_file, 'r') as svfile:
                v = vfile.read().rstrip()
                sv = svfile.read().rstrip()
